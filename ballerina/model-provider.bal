...
public isolated client class OpenAiModelProvider {
    *ai:ModelProvider;
    ...
    # + temperature - The temperature for controlling randomness in the model's output. Reasoning
    #                 models such as the GPT-5 and o-series deployments only accept the default
    #                 sampling temperature; supply `()` to omit the parameter for those deployments.
    ...
    public isolated function init(@display {label: "Service URL"} string serviceUrl,
        ...
        chat:ApiKeysConfig auth = {api\-key: apiKey};
    ...
    isolated remote function chat(ai:ChatMessage[]|ai:ChatUserMessage messages, ai:ChatCompletionFunctions[] tools, string? stop = ())
        ...
        // sampling `temperature`; the caller controls whether one is sent by configuring the
        // `temperature` field (set it to `()` for those deployments). Other deployments keep the
        // configured value, preserving the existing behaviour.
        if temperature is decimal {
        ...
        chat:inline_response_200|error rawResponse =
            self.llmClient->/deployments/[self.deploymentId]/chat/completions.post(request, api\-version = self.apiVersion);
